    ...
    __global float *out) //Espai de memoria local       
{
   ...
   for(int n=get_local_id(0) ; n < 64 ; n += get_local_size(0))
	...
	matriu[n][s]= getValue(img,cols,i+n,j+s);
   ...
   //Procesament
    ...
    int iloca = get_local_id(1)*4; //Obtenir espais locals
    int jloca = get_local_id(0);
	...
	for (int m=0; m < 4; m++){
	  ...
	  for(int k = 0; k < MIDAP; k++){
		  ...
		  //float value =  matriu[iloca+k+m][jloca+l] -  getValue(pat,MIDAP,k,l);
		float value =  matriu[iloca+k+m][jloca+l] -  getValue(pat,MIDAP,k,l);
		  sum += value * value;
	      }
	  }
	  
	  // Pasar de global a local 
	  // j -> l 
	  // i -> k
    ...
    __global float *out)        
{
    ...
    for (int m=0; m < 4; m++){
	  ...
	  for(int l = 0; l < MIDAP; l++){
	      ...
	      float value =  getValue(img,cols,i+k+m,j+l) -  getValue(pat,MIDAP,k,l); //lena ok
	      sum += value * value;
	  }
      }
      val =sum / (float) (MIDAP*MIDAP);
