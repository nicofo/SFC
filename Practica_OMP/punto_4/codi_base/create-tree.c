...
List *processPlainFile(char *bytes, int nbytes)
{
  //FILE *fp;
  ListData *listData;
  List *hashTable;
  ...
  char paraula[MAXCHAR], *paraula_copy;
  int i, j, len, is_word, valor_hash, done;
...
void processTextFiles(char *filename, RBTree *tree)
{
  ...
  char **bytes; //Para tener todos los archivos leidos
  ...
  /* Read all files intro of vector char * */
  
  lista=malloc(nfiles * sizeof(char *)); //Crear un vector de nombres de los fichero
  
  //hashTable = malloc(nfiles * sizeof(List *)); //Crear un vector de las tablas hash
  //Punto 4
  bytes=malloc(nfiles * sizeof(char *)); //crea un vector con todos los documentos Punto 4 
...
#pragma omp parallel firstprivate(nfiles) shared(lista,tree,bytes) 
{
    ...
    #pragma omp single
        ...
        for(i = 0; i < nfiles; i++) {
            ...
            //Leactura i guardar el fichero leido
            FILE *fp;
            ...
            bytes[i] = malloc(sizeof(char) * nbytes);
            if (!bytes[i]) {
                printf("No he pogut reservar memoria.\n");
                exit(1);
            }

            fread(bytes[i], sizeof(char), nbytes, fp);
            fclose(fp);
            

            /* Process file */

            #pragma omp task  firstprivate(i,nbytes) shared(bytes) private(hashTable)
            {
                printf("Processant fitxer %s\n", lista[i]);
                
                //printf("Longitud fitxer %i => %li Tamaño %i\n",i,strlen(bytes[i]),nbytes);
                
                /* Liberar espació de los ficheros cargados al final de procesarlos */
                
                hashTable = processPlainFile(bytes[i],nbytes);
...
//Alliberar tot l'espai de lectura
free(bytes);
